...
function extractGroupType(groupName: string): string | null {
  ...
  if (type.toLowerCase().includes('group')) return null  // grupos van como subtablas, no como tabs
  if (type.toLowerCase().includes('serie')) return type
  return type
}

const TAB_PRIORITY = ['Apertura', 'Intermedio', 'Clausura', 'Anual']
...
const currentSeason = season
  ? (
      // Primero intentar por año (ej: "2025")
      tournament.seasons.find(s => s.year === season) ??
      // Luego por ID numérico (backward compat)
      tournament.seasons.find(s => s.id === parseInt(season)) ??
      tournament.seasons.find(s => s.isCurrent) ??
  ...
  const hasData = Object.keys(standings).length > 0 || fixture.length > 0
  ...
  if (!type) {
    // Grupos tipo "Group A", "Group B" o "total" — agrupar juntos como subtablas
    if (!groupTypeMap['_grupos']) groupTypeMap['_grupos'] = []
    groupTypeMap['_grupos'].push(groupName)
    continue
  }
...
const availableTypes = Object.keys(groupTypeMap).filter(t => t !== '_grupos')
const defaultType = TAB_PRIORITY.find(t => availableTypes.includes(t)) ?? availableTypes[0] ?? '_grupos'
const activeGroupType = grupo ?? defaultType
const activeGroupNames = groupTypeMap[activeGroupType] ?? groupTypeMap['_grupos'] ?? []
        ...
        {!hasData && (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-dim)' }}>
            ...
            <div style={{ fontSize: 16, color: 'var(--text-muted)', marginBottom: 8 }}>
              Sin datos para la temporada {currentSeason.year}
            </div>
            <div style={{ fontSize: 13, color: 'var(--text-dim)' }}>
              Seleccioná otra temporada en el menú desplegable
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 0, marginBottom: 16, borderBottom: '1px solid var(--border)' }}>
              {[
                ...
                <Link
                  key={t.key}
                  href={`/torneo/${slug}?tab=${t.key}&season=${currentSeason.year}`}
                  style={{
                    padding: '8px 20px',
                    fontSize: 13,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: tab === 'tabla' ? '1fr 320px' : '1fr', gap: 16 }}>
              ...
              {tab === 'tabla' && (
                <>
                  <div>
                    {/* TABS DE APERTURA / CLAUSURA / INTERMEDIO / GRUPOS */}
                    {hasTabs && (
                      <div style={{ display: 'flex', gap: 0, marginBottom: 12, borderBottom: '1px solid var(--border)', overflowX: 'auto' }}>
                        {availableTypes.map(type => (
                          <Link
                            key={type}
                            href={`/torneo/${slug}?tab=tabla&season=${currentSeason.year}&grupo=${encodeURIComponent(type)}`}
                            style={{
                              padding: '6px 16px',
                              fontSize: 12,
                ...
                <div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
                    {roundNumbers.map(r => (
                      <Link
                        key={r}
                        href={`/torneo/${slug}?tab=fixture&fecha=${r}&season=${currentSeason.year}`}
                        style={{
                          padding: '4px 10px',
                          borderRadius: 4,
